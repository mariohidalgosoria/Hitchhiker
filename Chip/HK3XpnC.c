...
int main(void) 
{
  ...
  FILE *dat=fopen("HK3XIcR.dat","w");//data file of the initial condition in the lattice, just for checking
  FILE *datos=fopen("HK3XTcR.dat","w");//data file for the final sizes in the lattice
  ...
  for(id=ini;id<=fin;id++) {
	...
	if(LA[j]>1){//mass at last 2
	  ...
	  iseed=-(long) rand();
	  c=ran2(&iseed);
	  if(c > 0.5){//if place monomer to the right
	  ...
	  }//endif place monomer to the right
	  else{// place monomer to left
	    if(j==0){//BC there is breaking but j=0
	      LA[N-1]=LA[N-1]+1;
	      //LA[j]=LA[j]-1;
	    }//end else BC j=0
	  ...
	  }//endelse place monomer to left

	  /*
	  //chippppppppppppppppppppppppppppp
	  ...
	  //chippppppppppppppppppppppppppppppppppppppppppppppp
	  */
