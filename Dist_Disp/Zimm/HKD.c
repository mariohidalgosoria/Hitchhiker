/*Gives the final position of intercating random walkers with Rouse 
...
run as: ./HKD > xZi.dat
then  xZ.dat has all the relative displacement for all the different
size molecules at the initial latice in HK3XTR1.dat
we run it 10 times in order to get apprx 10000 final displacements 
then for each tiem merge all the files into one 
./HKD > xZ1.dat
./HKD > xZ2.dat
....
./HKD > xZ17.dat
./HKD > xZ17.dat 
then merge all the files in one
cat xZ1.dat xZ2.dat xZ3.dat xZ4.dat xZ5.dat xZ6.dat xZ7.dat xZ8.dat xZ9.dat xZ10.dat xZ11.dat xZ12.dat xZ13.dat xZ14.dat xZ15.dat xZ16.dat xZ17.dat > xZtime.dat 
 */
#include <stdio.h>
#include <stdlib.h>
#include <time.h>
...
#define ID 618//number of different size molecules in  the lattice saved in HK3XTZ1.dat  
...
int X0[ID]={0};//array for the initial positions in the file HK3XTR1.dat
int main(void) 
{
  ...
  //with M=1000, for Rouse; M=600, for Zimm; M=2000 for Arrhenius
  //i.e. every M updates a monte carlo step is achieved 
  T=3000;//
 
  
  sprintf(nom,"HK3XTZ1.dat");//file with the initial configuration
  ...
  i=0;
  for(j=0;j<N;j++){//get the initial positions and store them in X[i]
    if(i<ID){
      if(LA[j] != 0) {
