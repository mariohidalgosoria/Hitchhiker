...
systems  of interacting random walkers on a 1D lattice employing,
the Single Molecule technique 
...
#define P 6000//number of particles; high density agg P/N 
...
int main(void) 
{
  ...
  int ini=1;
  int fin=10000;//number of independent single molecules tracked
  FILE *dat=fopen("HK3XIRz.dat","w");//data file of the initial condition in the lattice, just for checking
  FILE *datos=fopen("HK3XTRz.dat","w");//data file for the final sizes in the lattice
  ...
  for(id=ini;id<=fin;id++) {//initialize in each case the lattice with monomers
    for(i=0;i<N;i++){
     LA[i]=1;
    }
